...
func _process(delta):
	...
	if secondify:
		get_tree().call_group("right", "getOtherNotes", myPos)
	# Screen Exit
...
func slash():
	if slashable:
		...
		if hit:
			...
			if isDoubleFirst:
				if myPos - Global.deadCenter > Global.magicalDoubleMargin:
					hit = false
					secondify = false
					get_tree().call_group("right", "unSecond")
			if isDoubleSecond:
		...
		if hit:
			...
			get_tree().call_group("idleright", "register")
			var amount = 0
			if perfectHit:
				amount = 1.5
				$Perfect.show()
			...
			if isPlayerTurn:
				...
				var speed = 12
				while true:
					animation.position.x += speed * 0.573576436
					animation.position.y -= speed * 0.819152044
