...
func _process(delta):
	...
	if secondify:
		get_tree().call_group("left", "getOtherNotes", myPos)
	# Screen Exit
...
func slash():
	if slashable:
		...
		if hit:
			...
			if isDoubleFirst:
				if myPos - Global.deadCenter > Global.magicalDoubleMargin:
					hit = false
					secondify = false
					get_tree().call_group("left", "unSecond")
			if isDoubleSecond:
		...
		if hit:
			...
			get_tree().call_group("idleleft", "register")
			var amount = 0
			if perfectHit:
				amount = 1.5
				$Perfect.show()
			...
			if isPlayerTurn:
				...
				var speed = 17.5
				while true:
					animation.position.x += speed * 0.838670568
					animation.position.y -= speed * 0.544639035
