...
func _process(delta):
	...
	if secondify:
		get_tree().call_group("down", "getOtherNotes", myPos)
	# Screen Exit
...
func slash():
	if slashable:
		...
		if hit:
			...
			if isDoubleFirst:
				if myPos - Global.deadCenter > Global.magicalDoubleMargin:
					hit = false
					secondify = false
					get_tree().call_group("down", "unSecond")
			if isDoubleSecond:
		...
		if hit:
			...
			get_tree().call_group("idledown", "register")
			var amount = 0
			if perfectHit:
				amount = 1.5
				$Perfect.show()
			...
			if isPlayerTurn:
				...
				var speed = 13.25
				while true:
					animation.position.x += speed * 0.707106781
					animation.position.y -= speed * 0.707106781
